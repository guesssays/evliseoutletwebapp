...
//
// ENV:
//   TG_BOT_TOKEN               (обычно — новый токен)
//   TG_BOT_TOKEN_LIST          (необязательно; "NEW_TOKEN,OLD_TOKEN" — на время ротации)
...
//   └─ Клиент может присылать заголовок X-Bot-Username для диагностики «бот не тот»
...
// 🔎 вспомогательная для диагностики (безопасна)
...
/* ===== Токены: поддержка списка на время ротации ===== */
function getTokenList() {
  // Если есть TG_BOT_TOKEN_LIST — используем его. Иначе — TG_BOT_TOKEN как единственный элемент.
  const raw = (process.env.TG_BOT_TOKEN_LIST || process.env.TG_BOT_TOKEN || '')
    .split(',')
    .map(s => String(s || '').trim())
    .filter(Boolean);
  return raw;
...
function verifyTgInitData(rawInitData, reqId = '') {
  ...
  // некоторые прокси/браузеры могут класть initData в кавычки
  ...
  // 1) пробуем «как есть», перебором по токенам
  let r = null, usedToken = null;
  for (const tok of TOKENS) {
    const tryR = _parseAndCalc(tok, raw, reqId);
    if (tryR.ok) { r = tryR; usedToken = tok; break; }
  }

  // 2) если не ок — лечим «плюсы-как-пробелы» (+ → %20) и пробуем снова по каждому токену
  if (!r || !r.ok) {
    const fixed = raw.replace(/\+/g, '%20');
    if (fixed !== raw && DEBUG) logD(`[req:${reqId}] trying +→%20 fix`);
    for (const tok of TOKENS) {
      const r2 = _parseAndCalc(tok, fixed, reqId);
      if (r2.ok) { r = r2; usedToken = tok; raw = fixed; break; }
    }
  }

  // 3) если всё ещё не ок — падаем с подробной диагностикой
  if (!r || !r.ok) {
    if (DEBUG) {
      logD(`[req:${reqId}] initData mismatch`, {
        token_tail: tail(TOKENS[0] || ''),
  ...
  if (DEBUG) logD(`[req:${reqId}] tg ok with token *${tail(usedToken || '')} uid=${user.id}`);
...
async function fireAndForgetNotify(chatId, type, extra = {}) {
  try {
    ...
    await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Internal-Auth': process.env.ADMIN_API_TOKEN || '' },
...
function makeCore(readAll, writeAll) {
  ...
  return {
    ...
    async reserve(uid, pts, orderId, totalArg = 0, shortId = null) {
      ...
      // Если заказ уже есть и пришёл актуальный total — обновим запись
...
export async function handler(event) {
  ...
  if (DEBUG) {
    ...
    // лог какого бота видит сервер (для крайних случаев рассинхрона переменных)
